...
const inputShape = {
    imagePaths: z
        ...
        .min(1)
        .max(14)
        .describe("Absolute paths to 1–14 source images on disk (PNG/JPG/WebP). Used as visual reference: edit (1–2 images), style transfer (1–3), character consistency (up to 4 on Flash, 5 on Pro), object/product consistency (up to 10 on Flash, 6 on Pro), or compositing (any mix up to 14). >3 images auto-routes to Pro."),
    prompt: z
        .string()
        .min(1)
        .max(4000)
        .describe("What to change. Be specific and aim for one change at a time. Mention what should STAY the same (e.g. 'keep her face unchanged, change the dress from blue to red')."),
    aspectRatio: AspectRatioSchema.optional().describe("Output shape. Defaults to config. Use when you want to reframe (e.g. hero 16:9 → Pinterest 3:4)."),
...
export function makeEditImageHandler(deps) {
    return async (args) => {
        const refs = [];
        ...
        return filesResult(result.value.files, `Edited ${refs.length} reference image(s) into ${result.value.files.length} output(s) in ${result.value.durationMs}ms:`);
    };
}
