...
const submitShape = {
    requests: z
        ...
        .min(1)
        .max(100)
        .describe("Array of independent image-generation requests (1–100). Each entry is the same shape as generate_image's input. The job is processed asynchronously by Google; results come back in up to 24h at 50% of the per-image price."),
    displayName: z
        .string()
        .max(200)
        .optional()
...
export function makeBatchSubmitHandler(deps) {
    return async (args) => {
...
export function makeBatchGetHandler(deps) {
    return async (args) => {
        ...
        if (v.files.length === 0 && v.errors.length === 0) {
            return successResult(`Batch ${args.name} is in state ${v.state}.\nNo results yet — call again later. Typical completion: up to 24h.`);
        }
        const lines = [`Batch ${args.name} — state ${v.state}`];
        if (v.files.length > 0) {
...
export function makeBatchListHandler(deps) {
    return async () => {
        ...
        for (const j of result.value) {
            ...
            if (j.createTime)
                lines.push(`    created:     ${j.createTime}`);
            if (j.endTime)
                lines.push(`    ended:       ${j.endTime}`);
            if (j.error?.message)
                lines.push(`    error:       ${j.error.message}`);
            lines.push("");
        }
        return successResult(lines.join("\n").trimEnd());
    };
}
export function registerBatchTools(server, deps) {
    server.registerTool("batch_generate_image", {
...
Returns a job name like 'batches/abc123'. Use batch_get_job to poll status and download results.`,
    ...
    server.registerTool("batch_get_job", {
...
Idempotent: safe to call repeatedly. Call this every few hours after submitting a batch.`,
    ...
    server.registerTool("batch_list_jobs", {
