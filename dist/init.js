...
import { CONFIG_PATH, loadConfig, saveConfig, } from "./config/store.js";
import { AspectRatioSchema, ResolutionSchema, } from "./config/schema.js";
async function main() {
    ...
    console.log(chalk.dim("Get your Google AI Studio API key at https://aistudio.google.com/apikey\n"));
    const apiKey = await password({
        ...
        validate: (value) => {
            ...
            if (!value.startsWith("AIza"))
                return "Google API keys start with 'AIza'";
            if (value.length < 30)
                return "Key looks too short";
            return true;
        },
    });
    console.log(chalk.dim("Images will be saved under <project-dir>/Generations/Images/ (created automatically when you call the MCP from a project).\n"));
    ...
    console.log(`  1. Register the MCP with Claude Code:\n     ${chalk.bold("claude mcp add image-generation -- npx -y image-generation")}\n`);
    console.log(`  2. Install the Claude Code skills (so Claude knows when to use the tools):\n     ${chalk.bold("npx -y image-generation install-skills")}\n`);
    console.log(chalk.dim("Tip: if you are running from source, replace the bin name above with `node dist/server.js`.\n"));
}
main().catch((cause) => {
