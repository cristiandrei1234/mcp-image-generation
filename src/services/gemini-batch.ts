...
import { GoogleGenAI, type BatchJob, type InlinedResponse } from "@google/genai";
import mime from "mime";
import { err, ok, type Result } from "../infra/result.js";
import type { GeminiError } from "../infra/errors.js";
import type { Logger } from "../infra/logger.js";
import {
  buildGenerateContentArgs,
  classifyGeminiError,
  MODEL_ID,
  type GenerateOpts,
} from "./gemini-image.js";

export interface BatchSubmitOpts {
  requests: GenerateOpts[];
  displayName?: string;
}
...
export class GeminiBatchService {
  ...
  async submit(opts: BatchSubmitOpts): Promise<Result<BatchJobSummary, GeminiError>> {
  ...
  async get(name: string): Promise<Result<BatchJobSummary, GeminiError>> {
  ...
  async list(pageSize = 20): Promise<Result<BatchJobSummary[], GeminiError>> {
    const startedAt = Date.now();
    try {
  ...
  async cancel(name: string): Promise<Result<void, GeminiError>> {
    if (!isLikelyBatchName(name)) {
      return err({ kind: "bad_request", status: 400, message: `"${name}" is not a batch job name` });
    }
    const startedAt = Date.now();
  ...
  async getResults(name: string): Promise<Result<BatchResults, GeminiError>> {
    if (!isLikelyBatchName(name)) {
      return err({ kind: "bad_request", status: 400, message: `"${name}" is not a batch job name` });
    }
    const correlationId = randomUUID();
    ...
    const responses: InlinedResponse[] = job.dest?.inlinedResponses ?? [];
    const files: string[] = [];
    const errors: string[] = [];
...
export function isLikelyBatchName(value: string): boolean {
