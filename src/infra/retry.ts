...
import type { GeminiError } from "./errors.js";
...
const DEFAULTS: Required<Omit<RetryOptions, "logger">> = {
  maxAttempts: 3,
  baseDelayMs: 250,
  maxDelayMs: 10_000,
  jitter: true,
...
export function shouldRetry(error: GeminiError): boolean {
  switch (error.kind) {
    case "timeout":
    case "network":
    case "quota":
...
export async function withRetry<T>(
  fn: () => Promise<Result<T, GeminiError>>,
  opts: RetryOptions = {},
  ...
  let lastResult: Result<T, GeminiError> | undefined;
